...
def convertCloudFromOpen3dToRos(open3d_cloud, frame_id="world"):
    ...
    if not open3d_cloud.colors: # XYZ only
        ...
        colors = np.floor(np.asarray(open3d_cloud.colors)*255).astype(np.uint32) # nx3 matrix
        colors = colors[:,0] * BIT_MOVE_16 +colors[:,1] * BIT_MOVE_8 + colors[:,2]  
        # cloud_data=np.c_[points, colors]

        points_l = points.tolist()
        cloud_data = []
        for i in range(points.shape[0]):
            cloud_data.append(points_l[i] + [colors[i]])
    ...
    while received_ros_cloud is None and not rospy.is_shutdown():
        ...
        if 1: # Use the cloud from file
            ...
            ros_cloud = pc2.create_cloud(
                Header(frame_id="world"), FIELDS_XYZ , TEST_CLOUD_POINTS)
