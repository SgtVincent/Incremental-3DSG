...
class SceneGraphVisNode:

    def __init__(self, node_name='scene_graph_vis_node'):
        # ---------------------- ROS name/namespace ----------------- # 
        ...
        self.pub_room_nodes = rospy.Publisher('/scene_graph/scene_graph_vis_node/room_nodes', MarkerArray) 
        self.pub_room_names = rospy.Publisher('/scene_graph/scene_graph_vis_node/room_names', MarkerArray) 
        self.pub_edges_room2object = rospy.Publisher('/scene_graph/scene_graph_vis_node/edges_room2object', MarkerArray)
    ...
    def prepare_data(self, timeout=1):
        ...
        try: 
            obj_msg = rospy.wait_for_message(
                self.topic_bounding_volumes,MarkerArray, timeout=timeout)
        except rospy.exceptions.ROSException as e:
            print(f"Not received {self.topic_bounding_volumes} within {timeout} secs, continue...")
            obj_msg = None
        # TODO: change this method to call rosservice 
        try:
    ...
    def publish_room_layer(self, vis_mode="inplace", flag_vis_name=True):
        assert vis_mode in ["elavated", "inplace"]
        ...
        for room in self.room_layer.room_nodes:
            ...
            point_elavated = np.concatenate((room.pos[:2], [self.room_z_node]))
            marker = Marker(
                type=Marker.CUBE,
                id=room.room_idx,
                ...
                header=Header(frame_id=self.frame),
                color=ColorRGBA(0.0, 0.5, 1.0, 0.8),
                # text=text,
                )
            ...
            if flag_vis_name:
                point_elavated = np.concatenate((room.pos[:2], [self.room_z_text]))
    ...
    def publish_edges_room2object(self, vis_mode="inplace", color_map='viridis'):
        ...
        for room in self.room_layer.room_nodes:
            ...
            room_point = Point(room.pos[0], room.pos[1], self.room_z_node)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if obj_msg:
                self.object_layer = ObjectLayer.from_msg(obj_msg)
            if rseg_msg:
...
if __name__=='__main__':
    ...
    scene_graph_vis_node = SceneGraphVisNode()
